...
class FluxParkRunner:
    ...
    def __init__(self, cfg_core: flp.config.FluxParkConfig, input_hooks: Optional[Dict[str, Any]] = None):
        self.cfg = cfg_core
    ...
    def setup(self):
        ...
        (self.outdir, 
         ...
         self.indir_rasters,
         self.indir_masks, 
         self.intermediate_dir) = flp.setup.resolve_dirs(
            cfg.outdir, cfg.indir, cfg.indir_rasters,
            cfg.indir_masks, cfg.intermediate_dir)
             
        # compute grid parameters
        self.grid_params = flp.setup.compute_grid_params(
            cfg.x_min, cfg.x_max, cfg.y_min, cfg.y_max,
            cfg.cellsize, cfg.calc_epsg_code,
            self.indir_masks, cfg.mask)
        ...
        (self.luse_ids, 
         self.evap_ids,
         self.luse_label) = flp.setup.load_luse_evap_conv(self.indir)
        
        # read conversion table from luse
        (self.conv_output_table, 
         self.conv_output,
         self.conv_var) = flp.setup.load_conv_output(self.indir, cfg.output_mapping)
        ...
        (self.out_par_list, 
         ...
         self.out_var_list, 
         self.rerun_par_list,
         self.rerun_var_list) = flp.setup.prepare_output_and_rerun_lists(
            self.mods, cfg.output_files, self.conv_output_table, self.conv_output, 
            cfg.store_states)
        ...
        (self.dynamic_landuse,
         self.input_raster_years) = flp.setup.detect_dynamic_landuse_and_years(
        ...
        (self.imperv, 
         ...
         self.soil_cov_decid
            ...
            self.indir_rasters, self.grid_params, self.mods,
    ...
    def run(self):
        ...
        for i, date in enumerate(self.dates):
            ...
            int_cap, 
            soil_cov, 
            openwater_fact) = flp.prepgrids.apply_evaporation_parameters(
                self.luse_ids, self.evap_ids, self.evap_params, date.dayofyear, 
                landuse_map, self.imperv, mod_vegcover=cfg.mod_vegcover,
            ...
            eta, smdp, smda, prec_surplus = unsat_reservoirmodel(
                throughfall, soil_evap_act_est + trans_pot, old["smda"], soilm_scp, 
                soilm_pwp
            )
            ...
            daily_output = flp.postprocessing.post_process_daily(
                ...
                open_water_evap_act, smda, soilm_pwp, rain, etref, landuse_map, 
                prec_surplus
            )
            daily_output.update({
                "rain": rain,
            ...
            cum_output, old = flp.postprocessing.update_cumulative_fluxes(
                daily_output, old, date, cfg.reset_cum_day, cfg.reset_cum_month,
                self.cum_par_list, self.conv_output
            )
        ...
        logging.info(f"preparing rain {tot_time_rain_prep:.2f} sec, {tot_time_rain_prep/tot_time*100:.2f} %")
        logging.info(f"preparing etref {tot_time_etref_prep:.2f} sec, {tot_time_etref_prep/tot_time*100:.2f} %")
        logging.info(f"preparing vegetation data {tot_time_veg_prep:.2f} sec, {tot_time_veg_prep/tot_time*100:.2f} %")
        logging.info(f"calculation interception {tot_time_int_calc:.2f} sec, {tot_time_int_calc/tot_time*100:.2f} %")
        logging.info(f"calculation soil evaporation {tot_time_soilevap_calc:.2f} sec, {tot_time_soilevap_calc/tot_time*100:.2f} %")
        logging.info(f"calculation transpiration {tot_time_trans_calc:.2f} sec, {tot_time_trans_calc/tot_time*100:.2f} %")
        logging.info(f"post-processing {tot_time_postp:.2f} sec, {tot_time_postp/tot_time*100:.2f} %")
        logging.info(f"writing output {tot_time_writing:.2f} sec, {tot_time_writing/tot_time*100:.2f} %")
        logging.info(f"initialization and overhead {tot_time_overhead:.2f} sec, {tot_time_overhead/tot_time*100:.2f} %")
